...
const getWalletDetails = (SLPInstance, wallet) => {
  ...
  const Path245 = deriveAccount({ masterHDNode, path: "m/44'/465'/0'/0/0" });
  const Path145 = deriveAccount({ masterHDNode, path: "m/44'/147'/0'/0/0" });
  const PathZero = deriveAccount({ masterHDNode, path: "m/44'/0'/0'/0/0" });
  const Accounts = [Path245, Path145];

  return {
    mnemonic: wallet.mnemonic,
    cashAddresses: [Path245.cashAddress, Path145.cashAddress],
    slpAddresses: [Path245.slpAddress, Path145.slpAddress],
